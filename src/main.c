...
// moved to soundpacks.c
int list_sound_packs(void);

// moved to soundpacks.c
void get_user_keyboard_audio_dir(char *buffer, size_t buflen);

// moved to soundpacks.c
void get_user_mouse_audio_dir(char *buffer, size_t buflen);
...
int main(int argc, char *argv[]) {
  char *sound_name =
      strdup("eg-oreo"); // may be replaced by strdup; track for free
  char *mouse_sound_name =
      strdup("ping"); // may be replaced by strdup; track for free
    ...
    if (access(user_cfg_path, R_OK) == 0) {
      ...
      if (read_user_config(user_cfg_path, &cfg_keyboard_sound, &cfg_mouse_sound,
        ...
        if (cfg_mouse_sound) {
          ...
          mouse_sound_name =
              cfg_mouse_sound; // now owned, must free on exit paths
          mouse_sound_name_owned = 1;
        }
        volume = cfg_keyboard_volume; // Use keyboard volume as main volume for
                                      // CLI compatibility
      }
    }
  }
  ...
  while ((opt = getopt_long(argc, argv, "S:M:V:K:O:lhdsm::u::v", long_options,
    ...
    case 'm':
      if (optarg == NULL) {
        // No argument provided, mute both
        cli_keyboard_mute = 1;
        cli_mouse_mute = 1;
    ...
    case 'u':
      if (optarg == NULL) {
        // No argument provided, unmute both
        cli_keyboard_mute = 0;
        cli_mouse_mute = 0;
  ...
  // Initialize global mute from runtime file so child inherits correct state
  ...
  // Persist runtime mute state so audio child can pick it up
  ...
  // If only CLI control options were provided and not daemon/list/stop, exit
  // quietly
  ...
  while (1) {
    if (reload_requested) {
      ...
      if (get_user_config_path(user_cfg_path3, sizeof(user_cfg_path3))) {
        ...
        if (read_user_config(user_cfg_path3, &cfg_keyboard_sound,
                             &cfg_mouse_sound, &cfg_keyboard_volume,
                             &cfg_mouse_volume)) {
          printf("Read config - keyboard:%s mouse:%s kvol:%d mvol:%d (current "
                 ...
                 cfg_keyboard_volume, cfg_mouse_volume, current_keyboard_volume,
          ...
          if (cfg_keyboard_sound &&
            ...
            keyboard_changed = 1;
            strncpy(current_sound_name, cfg_keyboard_sound,
          ...
          if (cfg_mouse_sound &&
            ...
            mouse_changed = 1;
            strncpy(current_mouse_sound_name, cfg_mouse_sound,
                    sizeof(current_mouse_sound_name) - 1);
            current_mouse_sound_name[sizeof(current_mouse_sound_name) - 1] =
                '\0';
          }
          ...
          if (keyboard_changed || mouse_changed || keyboard_volume_changed ||
            ...
            int keyboard_valid =
                validate_keyboard_sound_pack(current_sound_name);
            int mouse_valid =
                validate_mouse_sound_pack(current_mouse_sound_name);

            if (keyboard_valid && mouse_valid) {
              // Update keyboard paths
              if (build_paths_for_keyboard_sound(
                      current_sound_name, current_config_path,
                      sizeof(current_config_path), current_sound_dir,
                      sizeof(current_sound_dir))) {
                // Update mouse paths
                if (build_paths_for_mouse_sound(
                        current_mouse_sound_name, current_mouse_config_path,
                  ...
                  start_children(current_sound_dir, current_config_path,
                                 ...
                                 current_mouse_config_path,
                                 current_mouse_volume, current_keyboard_mute,
                                 current_mouse_mute);
                  printf("Reloaded successfully: keyboard=%s, mouse=%s, "
                         "kvol=%d, mvol=%d\n",
